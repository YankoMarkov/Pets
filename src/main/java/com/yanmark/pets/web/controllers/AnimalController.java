...
public class AnimalController extends BaseController {
  ...
  ModelAndView all(ModelAndView modelAndView) {
    List<AnimalViewModel> animalViewModels =
        this.animalService.getAllAnimals().stream()
  ...
  ModelAndView createConfirm(
      @Valid @ModelAttribute("animalCreate") AnimalCreateBindingModel animalCreate,
      BindingResult bindingResult) {
    if (bindingResult.hasErrors()) {
      return this.view(CREATE_ANIMAL);
    }
    this.animalService.saveAnimal(animalCreate);
  ...
  @PreAuthorize("hasAnyAuthority('ADMIN','MODERATOR')")
  ModelAndView edit(
      @ModelAttribute("animalCreate") AnimalCreateBindingModel animalCreate,
      @PathVariable String id,
      ModelAndView modelAndView) {
    AnimalServiceModel animalServiceModel = this.animalService.getAnimalById(id);
    AnimalViewModel animalViewModel =
        this.modelMapper.map(animalServiceModel, AnimalViewModel.class);
