...
public class HomeServiceImpl implements HomeService {
  ...
  public HomeServiceImpl(
      PetService petService, AnimalService animalService, UserService userService) {
  ...
  @Override
  public Page<PetServiceModel> takePetsByUser(
      String animalId, ModelAndView modelAndView, Principal principal, HttpServletRequest request) {
    UserServiceModel userServiceModel = this.userService.getUserByUsername(principal.getName());
    Page<PetServiceModel> petServiceModels;
    if (animalId != null && animalId.length() > 0) {
      AnimalServiceModel animalServiceModel = this.animalService.getAnimalById(animalId);
      petServiceModels =
          this.petService.getAllPetsByOwnerAndAnimal(userServiceModel, animalServiceModel, request);
  ...
  @Override
  public Page<PetServiceModel> takeAllPets(String userId, HttpServletRequest request) {
    Page<PetServiceModel> petServiceModels;
    if (userId != null && userId.length() > 0) {
      UserServiceModel userServiceModel = this.userService.getUserById(userId);
      petServiceModels = this.petService.getAllPetsByOwner(userServiceModel, request);
    } else {
      petServiceModels = this.petService.getAllPets(request);
    }
    return petServiceModels;
  }
}
